...
def EvalModel(i):
	...
	score = model.evaluate(tr_batch, label_batch, show_accuracy=True, batch_size=nb_batch,verbose=0)
...
def ComputeFitness(pop,min_loss):
	...
	#####
	p = len(X_train)/nb_batch - 1
	j = int(math.floor(np.random.rand() * p + 0.5) * nb_batch)
	#####
...
while generation_count < max_gen: #loss_delta > 0.001:
	...
	if same_mi_counter > 5:
		mutation_factor = mutation_factor / 10.0
		same_mi_counter = 0
